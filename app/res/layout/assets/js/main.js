(() => {
    ...
    var pwa = () => {
        /**
         * Get full URL for this relative path
         * 
        ...
        /**
         * Initialize the Application Manifest
         */
        var manifest = () => {
            ...
            var myDynamicManifest = {
                "description": 'WordPress Code Review | Potrivit v.__VERSION__',
                "short_name": 'Potrivit',
                "name": "Potrivit",
                "theme_color": "#13d213",
                ...
                "icons": [
                    {
                        "src": getPath('__FOLDER_MAIN__/img/64.png'),
                        "sizes": "64x64",
                        "type": "image/png",
                        "purpose": "maskable any"
                    },
                    {
                        "src": getPath('__FOLDER_MAIN__/img/192.png'),
                        "sizes": "192x192",
                        "type": "image/png",
                        "purpose": "maskable any"
                    },
                    {
                        "src": getPath('__FOLDER_MAIN__/img/512.png'),
                        "sizes": "512x512",
                        "type": "image/png",
                        "purpose": "maskable any"
                    }
            ...
            var manifestURL = URL.createObjectURL(
                new Blob(
                    [JSON.stringify(myDynamicManifest)], 
        ...
        if ('__DOMAIN__' === window.location.hostname) {
            workers();
            manifest();
        }
    };
    ...
    var layout = () => {       
        ...
        document.querySelectorAll('[data-role="search"]').forEach((item) => {
            item.innerHTML = '<input type="text" autocomplete="off" id="search" placeholder="WordPress plugin slug name"/>'
            ...
            var search = { 
                perform: (data, string) => {
                    ...
                    for (var pluginSlug in results) {
                        resultString += `<a href="/__FOLDER_PLUGIN__/${pluginSlug}">${results[pluginSlug]}</a>`;
                    }
            ...
            item.addEventListener('keyup', function(e) {
                ...
                if (string.length >= 1) {
                    if ("undefined" !== typeof cache[string[0]]) {
                        ...
                        fetch(`/__FOLDER_MAIN__/json/${string[0]}.json`).then((res) => {
                            if (res.ok) {
                                return res.json();
                            }
        ...
        document.querySelectorAll('[data-role="commits"]').forEach((item) => {
            try {
                ...
                var timeStored = parseInt(
                    new Date(
                        `${itemData[0].replace(/\d{4}$/g, '')}-${itemData[0].replace(/^\d{4}|\d{2}$/g, '')}-${itemData[0].replace(/^\d{6}/g, '')}`
                    ).getTime() / 1000, 
                    10
                );
                ...
                for (var week = 1; week <= 52; week++) {
                    ...
                    for (var day = 1; day <= 7; day++) {
                        ...
                        stringSvg += `<rect width="11" height="11" x="${17 - week}" y="${15 * (day - 1)}" rx="2" ry="2"${dayClass.length ? (' class="' + dayClass + '"') : ''} data-count="${dayCommits}" data-date="${
                            new Intl.DateTimeFormat('en-US', {day:'numeric',month:'short',year:'numeric'}).format(dayDate)
                        }"></rect>`;
                ...
                var listeners = {
                    mouseover: (item, e) => {
                        svgTitle.innerHTML = `<b>${item.getAttribute('data-count')} ${(1 === parseInt(item.getAttribute('data-count'), 10) ? 'commit' : 'commits')}</b> on ${item.getAttribute('data-date')}`;
                ...
                item.querySelectorAll('rect').forEach((item) => {
                    item.addEventListener('mouseover', (e) => {listeners.mouseover(item, e)});
                });
        ...
        document.querySelectorAll('[data-role="radial"]').forEach((item) => {
            ...
            var svg = `<div>
                <svg viewBox="25 25 50 50" width="0" style="filter:hue-rotate(${itemHue}deg)">
                    <circle cx="50" cy="50" r="20" fill="#fff" stroke="#e0e0e0" stroke-width="1" />
                    <circle class="svg-stroke" cx="50" cy="50" r="20" fill="none" stroke="#13d213" stroke-width="2" />
                </svg>
                <span>${itemText}</span>
            </div>`;
        ...
        '__DOMAIN__' !== window.location.hostname && document.querySelectorAll('a').forEach((item) => {
            var href = item.getAttribute('href');
            if ("string" === typeof href) {
                item.setAttribute('href', href.replace(/^https?\:\/\/__DOMAIN__/g, ''));
            }
        });
        ...
        document.querySelectorAll('a[rel~="author"]').forEach((item) => {
            ...
            item.innerHTML = '<svg viewport="0 0 200 75" height="75" xmlns="http://www.w3.org/2000/svg">'
